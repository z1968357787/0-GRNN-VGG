...
class GraphLearner(nn.Module):
    def __init__(self, v_features, q_features, mid_features, dropout=0.0, sparse_graph=True):
        ...
        self.lin1 = FCNet(v_features+q_features*2, mid_features, activate='relu')
        self.lin2 = FCNet(mid_features, mid_features, activate='relu')

    def forward(self, v, q, difficulty_level, v_mask, top_K):
        '''
        Input:
...
class GraphConv(nn.Module):
    ...
    def get_gaussian_weights(self, coord):
        ...
        # normalise weights
        weights = weights / (
        torch.sum(weights, dim=1, keepdim=True) + 1e-14)  # batch*num_obj*num_obj,  n_kernels (sum=-1)
...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size, vision_features=500, dropout=0.5):
        """
        ...
        self.encoder_cnn = EncoderCNN(vision_features)
        ...
        self.top_down_attention = nn.GRUCell(embed_dim, decoder_dim, bias=True) # top down attention LSTMCell
    ...
    def forward(self, v, encoded_questions, question_lengths, difficulty_level=None, adj=None):
        """
        Forward propagation.
        ...
        h1 = self.encoder_cnn(v)
        embeddings = self.embedding_dropout(self.embedding(encoded_questions))  # (batch_size, max_caption_length, embed_dim)
        ...
        for t in range(max(decode_lengths)):
            ...
            h1 = self.top_down_attention(
                embeddings[:batch_size_t, t, :], h1[:batch_size_t])
            preds = self.fc1(self.fc_dropout(h1))
