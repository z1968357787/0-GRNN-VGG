...
decoder_dim = 500  # dimension of decoder RNN
dropout = 0.5
...
def train(train_loader, decoder, criterion_ce, criterion_dis, decoder_optimizer, epoch):
    ...
    for i, (imgs, ques, ans, item, q_length, image_id) in enumerate(train_loader):
        ...
        # Forward prop.
        scores, scores_d, ques_sorted, decode_lengths, sort_ind = decoder(imgs, ques, q_length)
        ...
        scores = pack_padded_sequence(scores, decode_lengths.cpu(), batch_first=True, enforce_sorted=False).data
        targets = pack_padded_sequence(targets, decode_lengths.cpu(), batch_first=True, enforce_sorted=False).data
...
def validate(val_loader, decoder, criterion_ce, criterion_dis, word_map):
    ...
    # Batches
    with torch.no_grad():
        for i, (imgs, ques, ans, item, q_length, image_id) in enumerate(val_loader):
